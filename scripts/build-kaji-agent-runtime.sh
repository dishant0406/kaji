...
if [[ ! -d "$RUNTIME_ROOT" ]]; then
    echo "Error: Kaji runtime not found at $RUNTIME_ROOT" >&2
    exit 1
fi

if ! command -v bun >/dev/null 2>&1; then
    echo "Error: Bun is required to build the Kaji runtime. Install with: curl -fsSL https://bun.sh/install | bash" >&2
    exit 1
fi

if [[ ! -d "$RUNTIME_ROOT/node_modules" ]]; then
    echo "==> Installing Kaji runtime dependencies"
    bun install --cwd "$RUNTIME_ROOT"
fi

echo "==> Type-checking Kaji runtime"
bun run --cwd "$RUNTIME_ROOT" check:types

mkdir -p "$OUT_DIR"

echo "==> Bundling Kaji runtime"
bun build \
...
echo "==> Kaji runtime ready in $OUT_DIR"
