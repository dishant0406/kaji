...
@MainActor
final class DroidBrowserControlRegistry {
    static let shared = DroidBrowserControlRegistry()

    private var sessions: [String: DroidBrowserSessionTarget] = [:]

    private init() {}

    func register(sessionID: String, state: BrowserPaneState, controllers: BrowserControllerRegistry, close: @escaping () -> Void) {
        sessions[sessionID] = DroidBrowserSessionTarget(state: state, controllers: controllers, close: close)
    }
    ...
    func handle(_ command: DroidBrowserControlCommand) async -> String {
        guard let target = sessions[command.sessionID] else {
            return DroidBrowserControlJSON.body([
                "connected": false,
        ...
        return DroidBrowserControlJSON.body(result)
    }

    private func run(_ command: DroidBrowserControlCommand, target: DroidBrowserSessionTarget) async -> [String: Any] {
    ...
    private func navigate(_ arguments: [String: String], target: DroidBrowserSessionTarget) -> [String: Any] {
    ...
    private func newTab(_ arguments: [String: String], target: DroidBrowserSessionTarget) -> [String: Any] {
    ...
    private func readPage(target: DroidBrowserSessionTarget) async -> [String: Any] {
    ...
    private func readableText(target: DroidBrowserSessionTarget) async -> String {
        for _ in 0 ..< 10 {
    ...
    private func readablePayload(text: String, target: DroidBrowserSessionTarget) -> String {
        ...
        return [
            ...
            "URL: \(page?.url ?? "")",
            "",
            trimmed.isEmpty ? "No page text was exposed by Chromium yet. Use droid_browser_screenshot for visual inspection." : text,
        ].joined(separator: "\n")
    }

    private func screenshot(target: DroidBrowserSessionTarget) -> [String: Any] {
    ...
    private func current(target: DroidBrowserSessionTarget) -> [String: Any] {
        let page = target.state.selectedPage
...
@MainActor
private struct DroidBrowserSessionTarget {
