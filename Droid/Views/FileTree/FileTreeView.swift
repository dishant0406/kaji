    ...
    private var header: some View {
        HStack(spacing: 8) {
            ...
            IconButton(
                ...
                color: state.showOnlyChanges ? DroidTheme.accent : DroidTheme.fgMuted,
                hoverColor: state.showOnlyChanges ? DroidTheme.accent : DroidTheme.fg,
...
private struct FileTreeRow: View {
    ...
    private var rowBackground: Color {
        if isDropHighlighted { return DroidTheme.accentSoft }
        if isSelected { return DroidTheme.accentSoft }
    ...
    private var dropOverlay: some View {
        if isDropHighlighted {
            RoundedRectangle(cornerRadius: 3)
                .stroke(DroidTheme.accent, lineWidth: 1)
                .padding(.horizontal, 4)
        }
    }

    private var icon: some View {
        DroidIcon(systemName: iconSymbol, size: 11)
    ...
    private var iconColor: Color {
        if entry.isDirectory { return DroidTheme.fgMuted }
        return statusColor ?? DroidTheme.fgMuted
    }

    private var textColor: Color {
        if let statusColor { return statusColor }
        if entry.isDirectory, state.directoryHasChanges(entry.absolutePath) {
            return DroidTheme.diffHunkFg
        }
...
private struct FileTreeNewEntryRow: View {
    ...
    var body: some View {
        HStack(spacing: 4) {
            Color.clear.frame(width: CGFloat(depth) * 12)
            DroidIcon(systemName: kind == .folder ? "folder" : "doc", size: 11)
