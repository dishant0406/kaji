...
struct ParentAgentHeaderControls: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            DroidPill(title: providerLabel, leadingIcon: "sparkles", trailingIcon: "chevron.down", variant: .filled) {
                NotificationCenter.default.post(name: .openParentAgentSettings, object: nil)
            }
            .help("Parent Agent settings")

            if showsNewThread {
                DroidPill(title: "New thread", leadingIcon: "plus", variant: .plain, action: onNewThread)
                    .help("Start a new parent-agent thread")
                DroidPill(title: "Copy thread", leadingIcon: "doc.on.doc", variant: .plain) {
                    copyThread()
                }
