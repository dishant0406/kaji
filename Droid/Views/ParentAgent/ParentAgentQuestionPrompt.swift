...
struct ParentAgentQuestionPrompt: View {
        ...
        VStack(alignment: .leading, spacing: 10) {
            HStack(alignment: .top, spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    MarkdownInlineText(content: pendingQuestion.question, size: 13, color: DroidTheme.fgMuted)
                }
                Spacer(minLength: 0)
            }
    ...
    private var optionPicker: some View {
        VStack(alignment: .leading, spacing: 10) {
                ...
                LazyVStack(alignment: .leading, spacing: 0) {
                    ForEach(Array(filteredOptions.enumerated()), id: \.element.id) { index, option in
                        ...
                        if option.id != filteredOptions.last?.id {
                            Rectangle()
                                .fill(DroidTheme.border.opacity(0.5))
                                .frame(height: 1)
                                .padding(.leading, 34)
                        }
                    }
            ...
            .background(DroidTheme.bg.opacity(0.62), in: RoundedRectangle(cornerRadius: 12))
            resultCount
        }
    }
    ...
    private var searchField: some View {
        ...
        .padding(.vertical, 8)
        .background(DroidTheme.bg.opacity(0.72), in: RoundedRectangle(cornerRadius: 10))
    }
        ...
        Button {
            ...
            HStack(spacing: 12) {
                Text("\(index + 1)")
                    ...
                    .background(DroidTheme.secondaryBackground, in: RoundedRectangle(cornerRadius: 8))
                ...
                Spacer(minLength: 0)
                DroidIcon(systemName: "arrow.right", size: 11)
                    .foregroundStyle(DroidTheme.fgDim)
            }
            .padding(.horizontal, 10)
            .padding(.vertical, 9)
