...
struct ParentAgentMarkdownText: View {
    ...
    private func blockView(_ block: ParentAgentMarkdownBlock) -> some View {
        ...
        case let .heading(level, text):
            MarkdownInlineText(content: text, size: headingSize(level), color: DroidTheme.fg)
                .fontWeight(.semibold)
        ...
        case let .quote(text):
            HStack(alignment: .top, spacing: 10) {
                Rectangle()
                    ...
                    .frame(width: 2)
                MarkdownInlineText(content: text, size: size, color: DroidTheme.fgDim)
            }
            .padding(.vertical, 2)
        case let .code(text):
            ...
            .background(DroidTheme.bg.opacity(0.7), in: RoundedRectangle(cornerRadius: 10))
            .overlay(RoundedRectangle(cornerRadius: 10).stroke(DroidTheme.border.opacity(0.7)))
        case .divider:
            Rectangle()
                .fill(DroidTheme.border.opacity(0.8))
                .frame(height: 1)
                .padding(.vertical, 4)
        }
    }
