...
struct DroidKitScriptRunnerView: View {
    let runner: DroidKitScriptRunner
    let onStop: () -> Void
    let onClose: () -> Void
    ...
    private var header: some View {
        HStack(spacing: 10) {
            DroidIcon(systemName: statusIcon, size: 13)
    ...
    private var outputView: some View {
        ScrollViewReader { proxy in
            ...
            .background(DroidTheme.surface.opacity(0.45))
    ...
    private var statusColor: Color {
        switch runner.status {
        case .failed: DroidTheme.diffRemoveFg
        case .succeeded: DroidTheme.diffAddFg
        default: DroidTheme.fgMuted
        }
    }
}
