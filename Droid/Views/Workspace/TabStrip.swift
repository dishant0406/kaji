...
private struct TabAddButton: View {
    ...
    var body: some View {
        Button(action: action) {
            ZStack {
                Rectangle()
                    .fill(hovered ? DroidTheme.surface : DroidTheme.bg)
            ...
            .overlay {
                Rectangle()
                    .strokeBorder(DroidTheme.border.opacity(hovered ? 0.75 : 0.35), lineWidth: 1)
            }
...
private struct TabCell: View {
    ...
    private var tabBackground: Color {
        ...
        if active { return DroidTheme.surface }
    ...
    private var bottomAccentColor: Color? {
        ...
        return tabColor ?? DroidTheme.accent
    }
    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            .overlay {
                Rectangle()
                    .strokeBorder(DroidTheme.border.opacity(active || hovered ? 0.75 : 0.35), lineWidth: 1)
            }
            .contentShape(Rectangle())
            .onHover { hovering in
            ...
            .droidPopover(isPresented: $showColorPicker, preferredEdge: .bottom) {
            ...
            Rectangle().fill(DroidTheme.border.opacity(0.55)).frame(width: 1)
        }
    ...
    private var tabIconView: some View {
        if tab.isPinned {
            DroidIcon(systemName: "pin.fill", size: 10)
        } else if tab.kind == .vcs {
            DroidIcon(systemName: "file.diff", size: 12)
            ...
            DroidIcon(systemName: "point.3.connected.trianglepath.dotted", size: 12)
        } else if tab.kind == .browser {
            DroidIcon(systemName: "globe", size: 12)
        } else {
