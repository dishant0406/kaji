...
enum MarkdownWebBridge {
    static let scrollHandlerName = "droidMarkdownScroll"

    static let scrollObserverScript = #"""
    (() => {
        const handler = window.webkit?.messageHandlers?.droidMarkdownScroll;
        if (!handler) return;
    ...
    static func scrollToTopScript(_ scrollTop: CGFloat) -> String {
        ...
        (() => {
            ...
            window.__droidProgrammaticScroll = true;
            root.scrollTop = target;
            setTimeout(() => { window.__droidProgrammaticScroll = false; }, 180);
        })();
        """
    }
}
