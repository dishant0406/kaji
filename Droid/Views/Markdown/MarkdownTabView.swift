...
private let markdownWebLogger = Logger(subsystem: "app.droid", category: "MarkdownWebView")
...
struct MarkdownWebView: NSViewRepresentable {
    ...
    final class Coordinator: NSObject, WKNavigationDelegate, WKScriptMessageHandler {
        ...
        private func collectJavaScriptErrors(from webView: WKWebView) {
            let script = """
            (() => {
                const entries = Array.isArray(window.__droidErrors) ? window.__droidErrors : [];
                window.__droidErrors = [];
                return entries;
            })()
            """
        ...
        private func logAnchorGeometryIssuesIfNeeded(_ snapshot: [MarkdownPreviewAnchorGeometry]) {
            guard UserDefaults.standard.bool(forKey: "DroidMarkdownAnchorGeometryDebug") else {
                return
            }
