...
enum MarkdownPreviewAnchorGeometryBridge {
    static let geometryHandlerName = "droidMarkdownAnchorGeometry"

    static let observerScript = #"""
    (() => {
        const handler = window.webkit?.messageHandlers?.droidMarkdownAnchorGeometry;
        if (!handler) return;
        ...
        const anchorSelector = '[data-droid-anchor-id]';
        ...
        const measure = reason => {
            ...
            for (const node of nodes) {
                const anchorID = node.getAttribute('data-droid-anchor-id');
                ...
                anchors.push({
                    anchorID,
                    startLine: readInt(node.getAttribute('data-droid-line-start')),
                    endLine: readInt(node.getAttribute('data-droid-line-end')),
                    top,
                    height,
                });
            }
        ...
        const attachImageListeners = container => {
            ...
            for (const image of images) {
                if (image.__droidGeometryListenerAttached) continue;
                image.__droidGeometryListenerAttached = true;
        ...
        window.__droidMeasureAnchors = reason => settle(reason || 'manual');
        connect();
    })();
    """#
    ...
    static func requestMeasureScript(reason: String) -> String {
        ...
        return """
        (() => {
            if (typeof window.__droidMeasureAnchors === 'function') {
                window.__droidMeasureAnchors("\(escaped)");
            }
        })();
        """
    }
