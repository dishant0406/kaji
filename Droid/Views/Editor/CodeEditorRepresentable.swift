...
struct CodeEditorView: NSViewRepresentable {
    ...
    final class Coordinator: NSObject, NSTextViewDelegate {
        ...
        private static let perfLogger = Logger(subsystem: "app.droid", category: "EditorPerf")
        private static let perfEnabled: Bool = {
            if let env = ProcessInfo.processInfo.environment["DROID_EDITOR_PERF"] {
                let value = env.lowercased()
                return value == "1" || value == "true" || value == "yes"
            }
            return UserDefaults.standard.bool(forKey: "DroidEditorPerf")
        }()
