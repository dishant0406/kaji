...
private struct DiffViewerBreadcrumb: View {
    ...
    var body: some View {
        HStack(spacing: 6) {
            FileDiffIcon()
                .stroke(DroidTheme.fgDim, style: StrokeStyle(lineWidth: 1.5, lineCap: .round, lineJoin: .round))
    ...
    private var modeToggle: some View {
        ...
        .overlay(RoundedRectangle(cornerRadius: 5).stroke(DroidTheme.border, lineWidth: 1))
    }
    ...
    private func modeButton(_ mode: VCSTabState.ViewMode, symbol: String, tooltip: String) -> some View {
        ...
        return Button {
            ...
            DroidIcon(systemName: symbol, size: 10)
                .foregroundStyle(selected ? DroidTheme.fg : DroidTheme.fgMuted)
                .frame(width: 22, height: 20)
                .background(selected ? DroidTheme.bg : Color.clear)
                .contentShape(Rectangle())
        }
        .buttonStyle(.plain)
        .help(tooltip)
