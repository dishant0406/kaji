...
struct DroidCodeGraphFlowCanvas: View {
    ...
    @State private var dragMode: DroidCodeGraphFlowDragMode?
    ...
    private func dragGesture(layout: DroidCodeGraphFlowLayout) -> some Gesture {
    ...
    private var zoomGesture: some Gesture {
        MagnificationGesture()
            .onChanged { value in
                scale = DroidCodeGraphScale.clamped(scaleStart * value)
            }
            .onEnded { value in
                scale = DroidCodeGraphScale.clamped(scaleStart * value)
                scaleStart = scale
    ...
    private func beginDrag(at location: CGPoint, layout: DroidCodeGraphFlowLayout) -> DroidCodeGraphFlowDragMode {
    ...
    private func selectNode(at location: CGPoint, layout: DroidCodeGraphFlowLayout) {
        if let nodeID = layout.node(at: graphPoint(from: location)) {
            onSelect(nodeID)
        }
    }
    ...
    private func zoomOut() {
        scale = DroidCodeGraphScale.clamped(scale - 0.15)
        scaleStart = scale
    }
    ...
    private func drawEdges(context: GraphicsContext, layout: DroidCodeGraphFlowLayout) {
    ...
    private func draw(_ edge: DroidCodeGraphEdge, context: GraphicsContext, layout: DroidCodeGraphFlowLayout) {
        ...
        context.stroke(path, with: .color(DroidCodeGraphPalette.edge(selected: selected)), lineWidth: selected ? 1.2 : 0.75)
    }

    private func drawNodes(context: GraphicsContext, layout: DroidCodeGraphFlowLayout, nodes: [DroidCodeGraphNode]) {
        for node in nodes.sortedForFlow(selectedNodeID: selectedNodeID) {
            draw(node, frame: layout.nodeFrames[node.id], context: context)
        }
    }

    private func draw(_ node: DroidCodeGraphNode, frame: CGRect?, context: GraphicsContext) {
        guard let frame else { return }
        let selected = node.id == selectedNodeID
        context.fill(
            Path(roundedRect: frame, cornerRadius: DroidShape.tileRadius),
            with: .color(selected ? DroidTheme.accentSoft.opacity(0.72) : DroidTheme.surface)
        )
        context.stroke(
            Path(roundedRect: frame, cornerRadius: DroidShape.tileRadius),
            with: .color(selected ? DroidTheme.accent : DroidTheme.border),
        ...
        context.fill(
            ...
            with: .color(DroidCodeGraphPalette.color(for: node.community))
        )
    }

    private func visibleEdges(layout: DroidCodeGraphFlowLayout) -> [DroidCodeGraphEdge] {
        document.edges.filter {
            layout.nodeFrames[$0.source] != nil && layout.nodeFrames[$0.target] != nil
        }.prefix(2400).map(\.self)
    }

    private var displayedNodes: [DroidCodeGraphNode] {
    ...
    private func isSelected(_ edge: DroidCodeGraphEdge) -> Bool {
    ...
    private func label(for node: DroidCodeGraphNode, selected: Bool) -> Text {
        Text(node.label).font(.system(size: 11, weight: selected ? .semibold : .medium))
            .foregroundColor(DroidCodeGraphPalette.label(selected: selected))
    }

    private func subtitle(for node: DroidCodeGraphNode) -> Text {
        Text(node.sourceFile ?? node.fileType).font(.system(size: 9.5, weight: .regular, design: .monospaced))
            .foregroundColor(DroidTheme.fgDim)
    }
}
