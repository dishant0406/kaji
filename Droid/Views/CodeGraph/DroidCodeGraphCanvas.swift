...
struct DroidCodeGraphCanvas: View {
    ...
    @State private var dragMode: DroidCodeGraphDragMode?
    @State private var nodeOffsets: [String: CGSize] = [:]

    var body: some View {
        GeometryReader { proxy in
            let layout = DroidCodeGraphLayout(document: document, size: proxy.size, nodeOffsets: nodeOffsets)
            Canvas { context, _ in
    ...
    private func drawEdges(context: GraphicsContext, layout: DroidCodeGraphLayout) {
        for edge in document.edges.prefix(2400) {
            ...
            context.stroke(
                path,
                with: .color(DroidCodeGraphPalette.edge(selected: selected)),
                lineWidth: selected ? 1.3 : 0.7
            )
        }
    }

    private func drawNodes(context: GraphicsContext, layout: DroidCodeGraphLayout) {
        for node in document.nodes.prefix(1800) {
            ...
            let color = DroidCodeGraphPalette.color(for: node.community)
            context.fill(Path(ellipseIn: rect), with: .color(color.opacity(selected ? 1 : 0.82)))
            context.stroke(Path(ellipseIn: rect), with: .color(DroidCodeGraphPalette.nodeStroke(selected: false)), lineWidth: 1)
            if selected {
                context.stroke(
                    Path(ellipseIn: rect.insetBy(dx: -3, dy: -3)),
                    with: .color(DroidCodeGraphPalette.nodeStroke(selected: true)),
                    lineWidth: 1.4
                )
            }
        }
    ...
    private func drawLabels(context: GraphicsContext, layout: DroidCodeGraphLayout) {
    ...
    private var labeledNodes: [DroidCodeGraphNode] {
    ...
    private func beginDrag(at location: CGPoint, size: CGSize) -> DroidCodeGraphDragMode {
        let graphPoint = graphPoint(from: location)
        let layout = DroidCodeGraphLayout(document: document, size: size, nodeOffsets: nodeOffsets)
    ...
    private func updateDrag(_ mode: DroidCodeGraphDragMode, translation: CGSize) {
        switch mode {
...
private enum DroidCodeGraphDragMode {
