...
enum AgentVerificationRunner {
    ...
    static func plan(for run: AgentRun, fileManager: FileManager = .default) -> Plan? {
    ...
    @MainActor
    static func verify(runID: UUID, store: AgentRunStore = .shared) {
        guard let run = store.run(id: runID), let plan = plan(for: run) else {
