...
struct ClaudeCodeAgentModule: CodingAgentModule, AIUsageProvider {
    ...
    func install(hookClientPath: String) throws {
        ...
        for (key, command) in commands {
            hooks[key] = Self.mergeHookArray(
                ...
                droidHook: Self.buildHookEntry(command: command, matcher: key == "PermissionRequest" ? "*" : "")
            )
        }
    ...
    private func hookCommand(hookClientPath: String, event: String) -> String {
        "\(ShellEscaper.escape(hookClientPath)) claude-hook \(event) # \(Self.droidMarker)"
    }
    ...
    private static func mergeHookArray(existing: [[String: Any]]?, droidHook: [String: Any]) -> [[String: Any]] {
