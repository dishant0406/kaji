...
private let markdownLogger = Logger(subsystem: "app.droid", category: "MarkdownPreview")
...
enum MarkdownRenderer {
    ...
    static func html(filePath: String?) -> String {
        ...
        <head>
            ...
            <title>\(title)</title>
            <script src="droid-asset://markdown/marked.min.js"></script>
            <style id="droid-base-style">
                :root {
                    color-scheme: light;
                    --bg: #FFFFFF;
                    --fg: #1F2328;
                ...
                * { box-sizing: border-box; margin: 0; padding: 0; }
                ...
                .markdown-body pre.droid-prehl code.droid-hl { color: var(--fg); }
            </style>
            <style id="droid-syntax-style"></style>
        </head>
        <body>
            <div id="content">
            ...
            <script>
                window.__droidImageBaseHost = "\(imageBaseHost)";
            </script>
            <script src="droid-asset://markdown/markdown-renderer.js"></script>
        </body>
        </html>
        """
    }
    ...
    static func updateScript(content: String) -> String {
        ...
        return """
        (() => {
            if (typeof window.__droidRenderMarkdown !== 'function') {
                return false;
            }
            return window.__droidRenderMarkdown("\(encodedPayload)");
        })();
        """
    }
    ...
    static func themeApplyScript(palette: Palette) -> String {
        ...
        (() => {
            ...
            const syntaxStyle = document.getElementById('droid-syntax-style');
            if (syntaxStyle) {
                syntaxStyle.textContent = `\(escapedSyntaxCSS)`;
            }
            window.__droidMermaidBaseTheme = "\(mermaidBaseTheme)";
            window.__droidMermaidUseThemeVariables = \(isDarkPreview ? "true" : "false");
            window.__droidMermaidThemeVariables = \(mermaidThemeJSON);
            if (typeof window.__droidRerenderMermaid === 'function') {
                window.__droidRerenderMermaid();
            }
        })();
        """
    }
