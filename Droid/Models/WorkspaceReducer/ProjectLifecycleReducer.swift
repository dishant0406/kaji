...
enum ProjectLifecycleReducer {
    ...
    static func selectProject(
        ...
        state.activeWorktreePath[projectID] = worktreePath
    }
    ...
    static func removeWorktree(
        ...
        if let replacement {
            ...
            state.activeWorktreePath[projectID] = replacement.path
            return
        }
           ...
           .min(by: { $0.worktreeID.uuidString < $1.worktreeID.uuidString })
        {
            ...
            state.activeWorktreePath[projectID] = WorkspaceReducerShared.projectPath(for: fallback, state: state)
            return
        }
    ...
    static func cycleProject(
        ...
        state.activeWorktreePath[project.id] = worktree.path
    }
}
