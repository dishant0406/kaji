(function () {
    window.__droidErrors = window.__droidErrors || [];
    window.addEventListener('error', function (event) {
        try {
            var target = event && event.target;
            if (target && target.tagName === 'SCRIPT') {
                window.__droidErrors.push({
                    type: 'script-load',
    ...
    async function ensureMermaidLoaded() {
        ...
        var urls = ['droid-asset://markdown/mermaid.min.js'];
    ...
    function parseSyncAnchors(content) {
        ...
        while (i < lines.length) {
            ...
            var endLine = end + 1;
            anchors.push({
                id: 'droid-anchor-' + String(sequence),
                kind: kind,
                startLine: startLine,
    ...
    function rewriteImageSource(image) {
        ...
        if (lower.startsWith('https://')) {
            ...
            if (encoded) {
                image.setAttribute('src', 'droid-md-remote://image/' + encoded);
            }
            return;
        }
        ...
        var baseHost = window.__droidImageBaseHost || '';
        if (!baseHost) {
            return;
        }
        var relative = trimmed.replace(/^\/+/, '');
        var encodedRelative = relative.split('/').map(encodeURIComponent).join('/');
        image.setAttribute('src', 'droid-md-image://' + baseHost + '/' + encodedRelative);
    }
    ...
    function assignAnchorMetadata(markdownRoot, anchors) {
        ...
        for (var i = 0; i < elements.length && anchorIndex < anchors.length; i++) {
            ...
            if (['mermaid', 'image', 'fencedCode', 'table'].includes(elementKind)) {
                ...
                wrapper.className = 'droid-anchor-block droid-anchor-kind-' + elementKind;
            ...
            target.setAttribute('data-droid-anchor-id', anchor.id);
            target.setAttribute('data-droid-line-start', String(anchor.startLine));
            target.setAttribute('data-droid-line-end', String(anchor.endLine));
        }
    }
    ...
    async function renderMarkdown(content) {
        ...
        content = content.replace(/```mermaid\s*\r?\n([\s\S]*?)```/g, function (match, code) {
            ...
            return '<div class="mermaid" id="' + id + '" data-droid-mermaid="true"></div>';
        });
        window.__droidMermaidDiagrams = diagramMap;
            ...
            try {
                ...
                if (mermaidReady && typeof mermaid !== 'undefined') {
                    var mermaidConfig = {
                        ...
                        theme: window.__droidMermaidBaseTheme || 'default',
                        flowchart: { htmlLabels: false }
                    };
                    if (window.__droidMermaidUseThemeVariables && window.__droidMermaidThemeVariables) {
                        mermaidConfig.themeVariables = window.__droidMermaidThemeVariables;
                    }
    ...
    window.__droidRenderMarkdown = function (base64Payload) {
    ...
    window.__droidRerenderMermaid = async function () {
        var diagrams = window.__droidMermaidDiagrams;
        ...
        try {
            ...
            if (window.__droidMermaidUseThemeVariables && window.__droidMermaidThemeVariables) {
                config.themeVariables = window.__droidMermaidThemeVariables;
            }
