...
function refresh() {
  if (refreshing || process.env.DROID_BROWSER_DISABLE_TOOL_CACHE_REFRESH === '1') return;
  refreshing = true;
  ...
  request(child, pending, nextId++, 'initialize', {
    ...
    capabilities: {},
    clientInfo: { name: 'droid-browser-tool-cache', version: '0.1.0' }
  }).then(() => {
...
function cachePath() {
  return path.join(os.homedir(), '.droid', 'browser', 'playwright-tools-cache.json');
}

function packageSpec() {
  return process.env.DROID_BROWSER_PLAYWRIGHT_PACKAGE || '@playwright/mcp@latest';
}
...
function cacheTtlMs() {
  return Number(process.env.DROID_BROWSER_PLAYWRIGHT_TOOL_CACHE_TTL_MS || 86400000);
}
