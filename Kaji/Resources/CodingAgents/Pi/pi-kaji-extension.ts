...
export default function (pi: ExtensionAPI) {
  ...
  const sendSession = (ctx: any, source: string) => {
    ...
    const body = JSON.stringify({
      ...
      cwd: sanitize(manager?.getCwd?.() || process.env.KAJI_WORKTREE_PATH),
      source: sanitize(source),
      projectID: process.env.KAJI_PROJECT_ID,
      worktreeID: process.env.KAJI_WORKTREE_ID,
      worktreePath: process.env.KAJI_WORKTREE_PATH || "",
    })
  ...
  const shouldPrompt = (event: any) => {
    if (process.env.KAJI_PI_PERMISSION_MODE !== "prompt") return false
