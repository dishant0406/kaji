...
def main() -> None:
    parser = argparse.ArgumentParser(prog="kajicodegraph")
...
def run_legacy(out: Path, mode: str) -> None:
    ...
    write_status(out, mode, False, 0, 0, 0, "legacy", "failed", "KajiCodeGraph now requires the Kaji Parent Agent.")
...
def run_finalize(project: Path, out: Path, work: Path, mode: str, build_id: str) -> None:
    ...
    kaji_graph = to_kaji_graph(project, graph_json, communities, labels)
    (out / "kaji-graph.json").write_text(json.dumps(kaji_graph, indent=2), encoding="utf-8")
    ...
    write_status(
        ...
        mode,
        True,
        len(kaji_graph["nodes"]),
        len(kaji_graph["edges"]),
        len(kaji_graph["communities"]),
        build_id,
        "complete",
        None,
    )
...
def to_kaji_graph(project: Path, graph_json: dict, communities: dict[int, list[str]], labels: dict[int, str]) -> dict:
    degree = {}
    ...
    nodes = [kaji_node(node, degree) for node in graph_json.get("nodes", [])]
    edges = [{
...
def kaji_node(node: dict, degree: dict) -> dict:
    community = node.get("community")
    if community is None:
        community = 0
    return {
...
def write_status(out: Path, mode: str, ok: bool, nodes: int, edges: int, communities: int, build_id: str, state: str, message: str | None) -> None:
    status = {
        ...
        "kajiGraphPath": str(out / "kaji-graph.json"),
