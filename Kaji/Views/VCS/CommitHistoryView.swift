...
struct CommitHistoryView: View {
    ...
    private var branchPromptOverlay: some View {
        if let prompt = pendingBranchHash.map({ NamePrompt(hash: $0) }) {
            KajiModalOverlay(onDismiss: dismissBranchPrompt) {
                NameInputSheet(
                    title: "Create Branch",
    ...
    private var tagPromptOverlay: some View {
        if let prompt = pendingTagHash.map({ NamePrompt(hash: $0) }) {
            KajiModalOverlay(onDismiss: dismissTagPrompt) {
                NameInputSheet(
                    title: "Create Tag",
...
private struct CommitRow: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(commit.subject)
                    .kajiFont(size: 12, weight: .regular)
    ...
    private func refBadge(_ ref: GitRef) -> some View {
        ...
        return HStack(spacing: 2) {
            KajiIcon(systemName: icon, size: 8)
            Text(ref.name)
        ...
        .padding(.vertical, 2)
        .background(color.opacity(0.1), in: RoundedRectangle(cornerRadius: KajiShape.badgeRadius))
    }
        ...
        VStack(spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                CreateWorktreeLabeledField(fieldLabel) {
                    KajiInput(placeholder: placeholder, text: $name, monospaced: true)
                        .onSubmit {
