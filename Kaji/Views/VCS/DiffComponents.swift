...
struct DiffRenderTheme: @unchecked Sendable {
    ...
    private static func buildRules() -> [DiffHighlightRule] {
        let definitions: [RuleDefinition] = [
            RuleDefinition(pattern: #"'(?:\\.|[^'\\])*'"#, color: KajiTheme.nsDiffString, options: []),
            RuleDefinition(pattern: #""(?:\\.|[^"\\])*""#, color: KajiTheme.nsDiffString, options: []),
            RuleDefinition(pattern: #"`(?:\\.|[^`\\])*`"#, color: KajiTheme.nsDiffString, options: []),
            RuleDefinition(pattern: #"\b\d+(?:\.\d+)?\b"#, color: KajiTheme.nsDiffNumber, options: []),
            RuleDefinition(pattern: #"//.*$"#, color: KajiTheme.nsDiffComment, options: [.anchorsMatchLines]),
        ]
