...
struct KajiAgentMarkdownText: View {
    ...
    @State private var cache = KajiAgentMarkdownViewCache()
...
@MainActor
private final class KajiAgentMarkdownViewCache {
    private var content: String?
    private var blocks: [KajiAgentMarkdownBlock] = []

    func blocks(for newContent: String) -> [KajiAgentMarkdownBlock] {
        guard content != newContent else { return blocks }
        content = newContent
        blocks = KajiAgentMarkdownParser.parse(newContent)
        return blocks
    }
}
