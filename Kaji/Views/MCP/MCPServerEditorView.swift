    ...
    private var form: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                field("Name") {
                    KajiInput(placeholder: "filesystem", text: $state.server.name, width: 360, monospaced: true)
                }
                ...
                if state.server.transport == .stdio {
                    field("Command") {
                        KajiInput(placeholder: "npx", text: $state.server.command, width: 360, monospaced: true)
                    }
                    field("Arguments") {
                        KajiTextArea(
                    ...
                    field("Environment") {
                        KajiTextArea(placeholder: "API_KEY=$API_KEY", text: $environmentText, minHeight: 92, monospaced: true)
                            .frame(height: 92)
                    }
                } else if state.server.transport == .remote {
                    field("URL") {
                        KajiInput(placeholder: "https://example.com/mcp", text: $state.server.url, width: 440, monospaced: true)
                    }
                    field("Headers") {
                        KajiTextArea(
                    ...
                    field("Bearer Token Env") {
                        KajiInput(placeholder: "API_TOKEN", text: bearerBinding, width: 260, monospaced: true)
                    }
                }
                if state.server.transport != .plugin {
