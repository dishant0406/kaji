...
struct KajiCodeGraphFlowLayout {
    let nodeFrames: [String: CGRect]
    let contentSize: CGSize
    private let nodeIDs: [String]

    init(document _: KajiCodeGraphDocument, nodes: [KajiCodeGraphNode], viewportSize: CGSize, nodeOffsets: [String: CGSize] = [:]) {
        ...
        let metrics = KajiCodeGraphFlowLayoutMetrics(
    ...
    private static func point(
        for node: KajiCodeGraphNode,
        index: Int,
        metrics: KajiCodeGraphFlowLayoutMetrics
    ) -> CGPoint {
...
private struct KajiCodeGraphFlowLayoutMetrics {
    let center: CGPoint
    let bounds: CGSize
    let cardSize: CGSize
    let padding: CGFloat
...
extension [KajiCodeGraphNode] {
    func uniquedByID() -> [KajiCodeGraphNode] {
        var seen = Set<String>()
