...
struct AgentMissionControlControlsView: View {
    ...
    private func replyComposer(_ onReply: @escaping (String) -> Void) -> some View {
        HStack(spacing: 6) {
            KajiInput(placeholder: "Reply to this run", text: $replyText, width: 230)
            Button("Send") {
                onReply(replyText)
                replyText = ""
                replying = false
    ...
    private func actionButton(
        ...
        Button(action: action) {
            HStack(spacing: 4) {
                KajiIcon(systemName: iconName, size: 9)
                Text(title)
            ...
            .padding(.vertical, 4)
            .background(
                KajiTheme.surface.opacity(0.34),
    ...
    private func actionColor(role: AgentMissionControlActionRole, capability: AgentRunCapability) -> Color {
        guard capability.isAvailable else { return KajiTheme.fgDim }
        switch role {
        case .neutral:
