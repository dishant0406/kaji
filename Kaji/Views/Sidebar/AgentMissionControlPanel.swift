...
struct AgentMissionControlPanel: View {
    ...
    private var header: some View {
        HStack(spacing: 8) {
            KajiIcon(systemName: "rectangle.stack", size: 12)
                .foregroundStyle(KajiTheme.fgMuted)
            Text("Agents")
                .kajiFont(size: 12, weight: .semibold)
                .foregroundStyle(KajiTheme.fg)
            Spacer()
            KajiBadge(text: "\(items.count)", variant: items.contains { $0.status == .needsAttention } ? .warning : .neutral)
    ...
    private var list: some View {
        ...
        .background(KajiTheme.bg.opacity(0.28))
    }
    ...
    private var emptyState: some View {
        VStack(spacing: 8) {
            Spacer()
            KajiIcon(systemName: "checkmark.circle", size: 22)
