...
actor HuggingFaceModelDownloader {
    ...
    func download(model: SpeechInputModel, progress: ProgressHandler? = nil) async throws {
        ...
        for (index, file) in files.enumerated() {
            let destination = model.cacheURL.appendingPathComponent(file.localPath)
            ...
            let downloader = SpeechModelFileDownloader(destination: destination) { written, _ in
    ...
    private func remoteFiles(for model: SpeechInputModel) async throws -> [SpeechModelRemoteFile] {
        ...
        try await collectFiles(path: model.subPath ?? "", model: model, output: &files)
    ...
    private func collectFiles(path: String, model: SpeechInputModel, output: inout [SpeechModelRemoteFile]) async throws {
        let items = try await treeItems(model: model, path: path)
        for item in items {
            if item.type == "directory" {
                try await collectFiles(path: item.path, model: model, output: &output)
            }
    ...
    private func collectRootAuxiliaryFiles(
        ...
        for item in items where item.type == "file" && names.contains((item.path as NSString).lastPathComponent) {
            output.append(SpeechModelRemoteFile(remotePath: item.path, localPath: item.path, size: Int64(item.size ?? 0)))
        }
    }
    ...
    private func authorizedRequest(url: URL) throws -> URLRequest {
        ...
        if let token = Self.huggingFaceToken { request.setValue("Bearer \(token)", forHTTPHeaderField: "Authorization") }
        return request
    }
