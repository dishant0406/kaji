...
enum KajiCodeGraphMCPBinaryInstaller {
    ...
    static func commandURL(homeDirectory: String = NSHomeDirectory()) -> URL {
        directory(homeDirectory: homeDirectory).appendingPathComponent("kaji-codegraph-mcp")
    }

    static func install(homeDirectory: String = NSHomeDirectory(), fileManager: FileManager = .default) throws -> URL {
    ...
    static func remove(homeDirectory: String = NSHomeDirectory(), fileManager: FileManager = .default) throws {
        let directory = directory(homeDirectory: homeDirectory)
        let command = commandURL(homeDirectory: homeDirectory)
        let support = directory.appendingPathComponent("kaji-codegraph", isDirectory: true)
    ...
    static func isInstalled(homeDirectory: String = NSHomeDirectory(), fileManager: FileManager = .default) -> Bool {
        let command = commandURL(homeDirectory: homeDirectory)
        let supportMain = directory(homeDirectory: homeDirectory).appendingPathComponent("kaji-codegraph/codegraph-main.js")
    ...
    private static func syncCommand(into directory: URL, fileManager: FileManager) throws {
        guard let source = KajiCodeGraphMCPResourceLocator.scriptPath(fileManager: fileManager) else {
            throw KajiCodeGraphMCPBinaryInstallerError.missingResource
        }
        let command = directory.appendingPathComponent("kaji-codegraph-mcp")
        ...
        try syncSupport(into: directory.appendingPathComponent("kaji-codegraph", isDirectory: true), fileManager: fileManager)
    }
    ...
    private static func syncSupport(into support: URL, fileManager: FileManager) throws {
        ...
        if let source = KajiCodeGraphMCPResourceLocator.supportDirectory(fileManager: fileManager) {
            try fileManager.copyItem(at: source, to: support)
        } else {
            let files = KajiCodeGraphMCPResourceLocator.supportFiles(fileManager: fileManager)
            guard !files.isEmpty else { throw KajiCodeGraphMCPBinaryInstallerError.missingResource }
...
enum KajiCodeGraphMCPBinaryInstallerError: LocalizedError {
    case missingResource

    var errorDescription: String? {
        "Kaji CodeGraph MCP resources are missing."
    }
}
