...
struct KajiBrowserMCPInstallOutcome: Equatable {
    let agentID: String
    let installed: Bool
    let detail: String
}

enum KajiBrowserMCPInstallService {
    static let serverName = "kaji-browser"
    static let supportedAgentIDs = ["codex", "claude", "opencode", "pi"]

    static func installAll(
        homeDirectory: String = NSHomeDirectory(),
        projectPath: String? = nil
    ) -> [KajiBrowserMCPInstallOutcome] {
        do {
            let command = try KajiBrowserMCPBinaryInstaller.install(homeDirectory: homeDirectory)
            return supportedAgentIDs.map { install(agentID: $0, command: command, homeDirectory: homeDirectory, projectPath: projectPath) }
        } catch {
            return supportedAgentIDs.map { KajiBrowserMCPInstallOutcome(agentID: $0, installed: false, detail: error.localizedDescription) }
        }
    }
    ...
    static func uninstallAll(
        ...
        do {
            try KajiBrowserMCPBinaryInstaller.remove(homeDirectory: homeDirectory)
            return outcomes
        } catch {
            return outcomes + [KajiBrowserMCPInstallOutcome(agentID: serverName, installed: false, detail: error.localizedDescription)]
        }
    }

    static func installedAgentIDs(homeDirectory: String = NSHomeDirectory(), projectPath: String? = nil) -> [String] {
        guard KajiBrowserMCPBinaryInstaller.isInstalled(homeDirectory: homeDirectory) else { return [] }
    ...
    private static func install(
        ...
        guard let location = userLocations(agentID: agentID, homeDirectory: homeDirectory, projectPath: projectPath).first else {
            return KajiBrowserMCPInstallOutcome(agentID: agentID, installed: false, detail: "No writable MCP config location")
        }
        do {
            ...
            return KajiBrowserMCPInstallOutcome(agentID: agentID, installed: true, detail: location.url.path)
        } catch {
            return KajiBrowserMCPInstallOutcome(agentID: agentID, installed: false, detail: error.localizedDescription)
        }
    }

    private static func uninstall(agentID: String, homeDirectory: String, projectPath: String?) -> KajiBrowserMCPInstallOutcome {
        let locations = userLocations(agentID: agentID, homeDirectory: homeDirectory, projectPath: projectPath)
        guard !locations.isEmpty else {
            return KajiBrowserMCPInstallOutcome(agentID: agentID, installed: false, detail: "No MCP config location")
        }
        do {
            ...
            return KajiBrowserMCPInstallOutcome(agentID: agentID, installed: false, detail: "Removed")
        } catch {
            return KajiBrowserMCPInstallOutcome(agentID: agentID, installed: true, detail: error.localizedDescription)
        }
    }
