...
extension ParentAgentController {
    func spawnAgent(_ message: ParentAgentEnvelope, toolID: String) async {
        ...
        guard let provider = AskProvider.resolveAnnotation(message.arguments?["provider"] ?? "") else {
            sendToolError(id: toolID, message: "spawn_agent requires a provider chosen with kaji.choose_agent.")
            return
        }
        ...
        guard let model, !model.isEmpty else {
            sendToolError(id: toolID, message: "spawn_agent requires a model chosen with kaji.choose_agent.")
            return
        }
