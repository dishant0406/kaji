...
private let logger = Logger(subsystem: "app.kaji", category: "UpdateService")
...
final class UpdateService {
    ...
    private static let latestReleaseURL = URL(string: "https://api.github.com/repos/dishant0406/kaji/releases/latest")
    private static let upgradeCommand = "brew update && brew upgrade --cask kajikit"
    ...
    private func fetchLatestRelease() async throws -> GitHubRelease {
        ...
        request.setValue("Kaji", forHTTPHeaderField: "User-Agent")
    ...
    private func showAvailableUpdate(_ release: GitHubRelease) {
        let alert = NSAlert()
        alert.messageText = "Kaji \(release.version) is available"
        alert.informativeText = "Kaji is distributed through GitHub releases and Homebrew. To update, run:\n\n\(Self.upgradeCommand)"
    ...
    private func showNoUpdate(_ release: GitHubRelease) {
        let alert = NSAlert()
        alert.messageText = "Kaji is up to date"
