...
enum CodingAgentShimEnvironment {
    static let pathKey = "KAJI_AGENT_SHIM_DIR"
    ...
    static func variables(
        ...
        values.append(contentsOf: KajiShellBootstrapInstaller.install(
            homeDirectory: homeDirectory,
    ...
    private static func valuesWithCodeGraph(
        ...
        guard context.store.isReady,
              let instructions = KajiCodeGraphInstructions.ensureFile(
        ...
        values.append((key: "KAJI_CODE_GRAPH_ROOT_DIR", value: context.store.rootDirectory.path))
        values.append((
            key: "KAJI_CODE_GRAPH_PROJECT_DIR",
            value: context.store.projectDirectory(projectID: context.projectID, worktreeID: context.worktreeID).path
        ))
        values.append(contentsOf: KajiCodeGraphInstructions.environment(
            projectID: context.projectID,
            worktreeID: context.worktreeID,
            store: context.store,
            fileManager: context.fileManager
        ))
        _ = KajiCodeGraphInstructions.ensureCodexBridge(
            projectID: context.projectID,
            worktreeID: context.worktreeID,
            store: context.store,
            fileManager: context.fileManager
        )
        _ = KajiCodeGraphInstructions.ensureClaudeBridge(
            projectID: context.projectID,
            worktreeID: context.worktreeID,
            store: context.store,
            fileManager: context.fileManager
        )
        if let config = KajiCodeGraphInstructions.ensureOpenCodeConfig(
            projectID: context.projectID,
            worktreeID: context.worktreeID,
            ...
            values.append((key: "KAJI_CODE_GRAPH_OPENCODE_CONFIG", value: config.path))
        }
        return values
    }
    ...
    private static func realExecutableVariables(
        ...
        [
            ("codex", "KAJI_REAL_CODEX"),
            ("claude", "KAJI_REAL_CLAUDE"),
            ("claude-code", "KAJI_REAL_CLAUDE_CODE"),
            ("opencode", "KAJI_REAL_OPENCODE"),
            ("pi", "KAJI_REAL_PI"),
