...
private let typographyLogger = Logger(subsystem: "app.kaji", category: "AppTypographySettings")
...
final class AppTypographySettings {
    ...
    private init() {
        fileURL = KajiFileStorage.fileURL(filename: "app-typography.json")
        load()
    }
    ...
    private func loadLegacySnapshotIfAvailable() {
        let legacyURL = KajiFileStorage.fileURL(filename: "editor-settings.json")
