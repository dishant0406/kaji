...
- Agent Mission Control: `AgentMissionControlPanel` is a sidebar footer popover that turns `AIActivityStore` and provider notifications into task-level rows. `AgentMissionControlSnapshotBuilder` classifies active runs, attention items, failures, completions, and notices; live provider transcript snippets arrive through `*_transcript` socket events and stay attached to the active run in memory. `AgentMissionControlNavigator` jumps from a row back to the owning pane or notification context.
