...
- Notifications: `NotificationStore`, `NotificationNavigator`, `NotificationSocketServer`, `NotificationIntegrationStore`, `NotificationEndpointSender`, `CodexSessionMonitor`, `CodexOutboundNotificationCoordinator`, and the AI-provider hook installers under `Droid/Services/Providers/`. Built-in Codex, Claude Code, and OpenCode integrations can emit into Droid from terminal panes or from external shells by falling back to the default app socket and routing empty-pane events to the active pane. Codex also has a local session-rollout monitor for interactive `codex-tui` and Codex Desktop turns, because current Codex releases write `task_complete` into `~/.codex/sessions` even when the external `notify` hook does not fire. Outbound Codex deliveries are briefly delayed so `ntfy` and webhook routes can prefer the richer interactive completion text over the generic shell-hook payload. Notification settings can also route normalized events to outbound HTTP destinations such as `ntfy` and generic webhooks with keychain-backed bearer tokens and source/event matching rules.
