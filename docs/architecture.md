...
- Notifications: `NotificationStore`, `NotificationNavigator`, `NotificationSocketServer`, and the AI-provider hook installers under `Droid/Services/Providers/`. Built-in Codex, Claude Code, and OpenCode integrations can emit into Droid from terminal panes or from external shells by falling back to the default app socket and routing empty-pane events to the active pane.
