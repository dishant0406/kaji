...
- DroidCodeGraph: `DroidCodeGraphStore`, `DroidCodeGraphInstaller`, `DroidCodeGraphRuntime`, `DroidCodeGraphVersionArchive`, `DroidCodeGraphInstructions`, `CodingAgentShimInstaller`, `DroidShellBootstrapInstaller`, and `CodingAgentShimEnvironment` manage the optional Graphify-backed code graph extension from Settings. The extension runtime lives under `~/.droid/extensions/droidcodegraph`, clones a pinned Graphify checkout, creates an isolated Python venv, and runs a Droid-owned adapter script that emits `graph.json`, `droid-graph.json`, `GRAPH_REPORT.md`, `analysis.json`, `manifest.json`, and `status.json` per project/worktree. Each graph build captures the current git branch, commit, and dirty state, annotates the latest graph JSON, and archives a versioned copy under the project/worktree `versions/` directory so old graphs can be reopened after newer builds. Droid renders `droid-graph.json` in a native SwiftUI graph tab, loads large graph documents off the main actor, exposes latest/commit versions in the graph sidebar, and creates an editable Droid-owned `AGENTS.md` under the extension project/worktree directory. Droid terminals prepend `~/.droid/bin` shims for `codex`, `claude`, `claude-code`, `opencode`, and `pi`, then use a Droid-only zsh bootstrap to source the user's real zsh files and restore the shim directory to the front after shell startup changes. Both footer-launched and manually typed provider commands receive Droid-only graph context without mutating project or global agent files. The shims preserve provider arguments, add the generated project/worktree graph directory for Claude and Codex, pass Pi its Droid instruction file through `--append-system-prompt`, and use a Droid-managed `OPENCODE_CONFIG` file with OpenCode's native `instructions` entry. Droid never runs Graphify's instruction-file installers or mutates project agent files.
...
- `~/.droid/extensions/droidcodegraph`: managed Graphify checkout, Python venv, adapter script, logs, extension state, latest per-project graph artifacts, and commit-linked graph versions
