...
| Package | Why It Matters For Kaji | Recommendation |
| --- | --- | --- |
...
| `pi-lens` | LSP, linters, formatters, type-checking, secret scan, read-before-edit guard, diagnostics after writes. | High-value but heavy. Make optional as a Kaji quality/diagnostics integration. |
| `pi-ask-user` | Structured `ask_user` tool with searchable options, multi-select, freeform input, timeout, and structured result details. | Kaji already has native ask-user UI. Align with this schema and render natively. |
| `pi-subagents` | Scout/planner/worker/reviewer/oracle delegation, chains, parallel runs, background jobs, worktree isolation. | Use as design reference. Avoid direct install until Kaji's orchestration boundaries are clear. |
...
| `pi-depo` | Declarative package manager for Pi packages, skills, hooks, and MCP servers via `kit.yml`. | Useful if Kaji gets a package marketplace/sync system. |
...
| `pi-markdown-preview`, `pi-mermaid`, `@walterra/pi-charts` | Better rendering for markdown, Mermaid, and charts. | Kaji should render natively; use these as references. |
| `pi-studio` | Two-pane browser workspace with prompt/response editing and previews. | Reference only. Kaji already owns native UI. |
| `pi-interactive-shell` | Runs other coding agents inside Pi overlays with supervision. | Not needed directly; Kaji already runs agents in native terminal panes. |

## Highest-Value Kaji Integrations
...
- Add a Kaji Parent Agent tool like `kaji.mcp` or expose Pi-compatible `mcp`.
...
- Add native Kaji parent tools for search/fetch first.
...
Kaji already has a native question prompt. Align it with `pi-ask-user` rather than inventing another shape.
...
- Full orchestration packages (`taskplane`, `pi-crew`, `pi-subagents`) as direct dependencies. Kaji already has a Parent Agent and child-agent model; adding a second orchestration layer risks duplicated state, unclear ownership, and conflicting worktree/session control.
- Heavy UI packages (`pi-studio`, `glimpseui`) as embedded UI. Kaji should remain native macOS UI.
- Social/productivity integrations like WhatsApp or Notion before coding-agent fundamentals are stable.

## Proposed Kaji Roadmap
...
Implement Kaji-native equivalents for:
...
- Evaluate loading Pi extension factories through Pi's `ResourceLoader`/extension API in Kaji's parent-agent runtime.
...
Create a curated Kaji-recommended package set:
