...
CREATE TRIGGER IF NOT EXISTS history_ai AFTER INSERT ON history BEGIN
	...
	getRecent(limit: number, cwd?: string): HistoryEntry[] {
		...
		try {
			const rows = cwd
				? (this.#db.prepare("SELECT id, prompt, created_at, cwd FROM history WHERE cwd = ? ORDER BY created_at DESC, id DESC LIMIT ?").all(cwd, safeLimit) as HistoryRow[])
	...
	search(query: string, limit: number, cwd?: string): HistoryEntry[] {
		...
		try {
			ftsRows = cwd
				? (this.#db.prepare("SELECT h.id, h.prompt, h.created_at, h.cwd FROM history_fts f JOIN history h ON h.id = f.rowid WHERE history_fts MATCH ? AND h.cwd = ? ORDER BY h.created_at DESC, h.id DESC LIMIT ?").all(ftsQuery, cwd, safeLimit) as HistoryRow[])
...
END;
	...
	#getSubstringStmt(tokenCount: number, scoped = false): Statement {
		const key = scoped ? -tokenCount : tokenCount;
		let stmt = this.#substringStmts.get(key);
		if (stmt) return stmt;
		const whereClause = Array(tokenCount).fill("prompt LIKE ? ESCAPE '\\' COLLATE NOCASE").join(" AND ");
		const cwdClause = scoped ? " AND cwd = ?" : "";
