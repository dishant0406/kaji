...
struct WorkspaceReducerTests {
    ...
    private func makeState(
        ...
        var state = WorkspaceState(
            ...
            activeWorktreePath: [projectID: worktreePath],
            workspaceRoots: [:],
            focusedAreaID: [:],
            focusHistory: [:]
        )
    ...
    @Test("selectProject stores selection without creating workspace")
    func selectProjectNew() {
        ...
        #expect(state.activeWorktreePath[projectID] == testPath)
        let key = WorktreeKey(projectID: projectID, worktreeID: worktreeID)
    ...
    @Test("selectWorktree switches selection without creating workspace")
    func selectWorktreeNew() {
        ...
        #expect(state.activeWorktreePath[projectID] == "/tmp/other")
    ...
    func removeProject() {
        ...
        #expect(state.activeWorktreePath[projectID] == nil)
        let key = WorktreeKey(projectID: projectID, worktreeID: worktreeID)
        #expect(state.workspaceRoots[key] == nil)
        #expect(state.focusedAreaID[key] == nil)
    ...
    func removeWorktreeWithReplacement() {
        ...
        #expect(state.activeWorktreePath[projectID] == "/tmp/replacement")
    ...
    @Test("createTab bootstraps workspace for selected project")
    ...
    @Test("createCommandSplit opens command in a new right-hand area")
    ...
    @Test("closeTab last tab keeps project when preference is enabled")
    func closeTabLastTabKeepsProject() {
        let projectID = UUID()
        let worktreeID = UUID()
        var state = makeState(projectID: projectID, worktreeID: worktreeID)
