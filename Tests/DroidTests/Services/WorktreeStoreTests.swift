...
struct WorktreeStoreTests {
    ...
    func worktreeLegacyDecodeDefaultsToDroid() throws {
        let json = """
        {
          "id": "\(UUID().uuidString)",
          "name": "feature-a",
        ...
        #expect(worktree.source == .droid)
        #expect(worktree.isExternallyManaged == false)
    }

    @Test("Worktree removal capability is derived from source and primary state")
    func worktreeRemovalCapabilityMatchesState() {
        ...
        let primary = Worktree(name: "Primary", path: "/tmp/primary", isPrimary: true)

        #expect(managed.canBeRemoved)
        #expect(!external.canBeRemoved)
        #expect(!primary.canBeRemoved)
    }
    ...
    @Test("refreshFromGit keeps missing Droid-managed worktrees")
    func refreshFromGitKeepsMissingDroidManagedEntries() async throws {
        let project = Project(name: "Repo", path: "/tmp/repo")
        let persistence = WorktreePersistenceStub(
            initial: [
                project.id: [
    ...
    func refreshFromGitResolvesSymlinkedPrimaryPath() async throws {
        let tempRoot = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("droid-tests-\(UUID().uuidString)", isDirectory: true)
