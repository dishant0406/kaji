...
struct WorktreeStoreTests {
    ...
    func worktreeLegacyDecodeDefaultsToKaji() throws {
        let json = """
        {
          "id": "\(UUID().uuidString)",
          "name": "feature-a",
    ...
    @Test("refreshFromGit keeps missing Kaji-managed worktrees")
    func refreshFromGitKeepsMissingKajiManagedEntries() async throws {
        let project = Project(name: "Repo", path: "/tmp/repo")
        let persistence = WorktreePersistenceStub(
            initial: [
                project.id: [
