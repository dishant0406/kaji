...
struct ThemeFileCodecTests {
    ...
    func slugifiesNamesForThemeIdentifiers() {
        #expect(ThemeFileCodec.slugify(" Midnight Kaji  ") == "midnight-kaji")
    ...
    func normalizesDraftHexValues() {
        let draft = ThemeDraft(
            ...
            colors: ThemeColorSet(
                ...
                palette: ThemeDraft.kajiDefaults.colors.palette.map { $0.lowercased() }
            )
        )
    ...
    func buildAndParseRoundTripPreservesMetadata() throws {
        let draft = ThemeDraft(
            name: "Noir Terminal",
            slug: "noir-terminal",
            colors: ThemeDraft.kajiDefaults.colors
        )
    ...
    func uniqueSlugAppendsNumericSuffix() {
        let slug = ThemeFileCodec.uniqueSlug("kaji", existing: ["kaji", "kaji-2"])
        #expect(slug == "kaji-3")
    }
}
