...
struct WorkspaceReducerTests {
    ...
    @Test("createFilePreviewTab opens a top-level workspace tab")
    func createFilePreviewTabCreatesWorkspaceTab() throws {
        let projectID = UUID()
        let worktreeID = UUID()
        var state = makeState(projectID: projectID, worktreeID: worktreeID)
        let key = WorktreeKey(projectID: projectID, worktreeID: worktreeID)
        ...
        _ = WorkspaceReducer.reduce(
            action: .createFilePreviewTab(projectID: projectID, areaID: nil, filePath: "/tmp/test/image.png", kind: .image),
            state: &state
        )
    ...
    func createCodeGraphTabCreatesWorkspaceTab() throws {
        ...
        _ = WorkspaceReducer.reduce(
            action: .createCodeGraphTab(AppState.CodeGraphTabRequest(
                projectID: projectID,
                worktreeID: worktreeID,
                worktreePath: testPath,
                graphURL: URL(fileURLWithPath: "/tmp/test/kaji-graph.json")
            )),
            state: &state
        )
        ...
        #expect(workspace.tabs.count == 2)
        #expect(activeTab.root.allAreas().count == 1)
        #expect(activeTab.activeContent?.kind == .codeGraph)
        #expect(activeTab.title == "Code Graph")
    }

    @Test("createCodeGraphTab selects and bootstraps the requested worktree")
    func createCodeGraphTabBootstrapsInactiveProject() throws {
        let firstProjectID = UUID()
        let firstWorktreeID = UUID()
        let secondProjectID = UUID()
        let secondWorktreeID = UUID()
        let secondPath = "/tmp/second"
        var state = makeState(projectID: firstProjectID, worktreeID: firstWorktreeID)
        ...
        _ = WorkspaceReducer.reduce(
            action: .createCodeGraphTab(AppState.CodeGraphTabRequest(
                ...
                graphURL: URL(fileURLWithPath: "/tmp/second/kaji-graph.json")
            )),
            state: &state
        )

        let workspace = try #require(state.workspaces[secondKey])
        let activeTab = try #require(workspace.activeTab)
        let area = try #require(activeTab.root.allAreas().first)

        #expect(state.activeProjectID == secondProjectID)
        #expect(state.activeWorktreeID[secondProjectID] == secondWorktreeID)
        #expect(state.activeWorktreePath[secondProjectID] == secondPath)
        #expect(area.projectPath == secondPath)
        #expect(activeTab.activeContent?.kind == .codeGraph)
    }

    @Test("createCodeGraphTab keeps only one graph tab alive globally")
    func createCodeGraphTabClosesExistingGraphTabs() throws {
        let firstProjectID = UUID()
        let firstWorktreeID = UUID()
        let secondProjectID = UUID()
        let secondWorktreeID = UUID()
        ...
        _ = WorkspaceReducer.reduce(
            action: .createCodeGraphTab(AppState.CodeGraphTabRequest(
                ...
                worktreePath: testPath,
                graphURL: URL(fileURLWithPath: "/tmp/first/kaji-graph.json")
            )),
            state: &state
        )
        _ = WorkspaceReducer.reduce(
        ...
        let firstWorkspace = try #require(state.workspaces[firstKey])
        let secondWorkspace = try #require(state.workspaces[secondKey])

        #expect(firstWorkspace.tabs.allSatisfy { $0.kind != .codeGraph })
        #expect(secondWorkspace.activeTab?.kind == .codeGraph)
    }
}
